...
class CustomInviteNavBar: UIView {
    ...
    func setupInviteBoxes(viewModel: FriendAndInviteViewModel) {
        clearInviteViews()
        for i in 0..<(viewModel.inviteList.count) {
            ...
            let name = viewModel.inviteList[i].name
            let normalText = "\n邀請你成為好友：）"
    ...
    private func saveOriginalFrames() {
        originalFrames = inviteViews.map { $0.frame }
    }
